...
sequenceDiagram
    ...
    U->>UI: 点击保存按钮
    UI->>BL: ExtractGridData()
    BL->>BL: 获取DataGridView的DataSource
    BL->>BL: 检查DataTable行状态
    BL-->>UI: 返回DataTable数据
    
    UI->>BL: CreateNewCase()
    BL->>BL: 分类处理不同状态的行
    Note over BL: DataRowState.Added - 新增行<br/>DataRowState.Modified - 修改行<br/>DataRowState.Deleted - 删除行
    
    BL->>DAL: SaveDataTableWithTransaction()
    DAL->>DAL: 分别处理不同状态的行
    
    alt 新增行处理
        DAL->>DB: INSERT语句
        Note over DAL: 自动填充系统字段<br/>状态="新登录"<br/>更新时间、更新人员
    else 修改行处理
        DAL->>DB: UPDATE语句
        Note over DAL: 自动更新系统字段<br/>状态="已修改"<br/>更新时间、更新人员
    else 删除行处理
        DAL->>DB: DELETE语句
        Note over DAL: 使用原始值删除
    end
