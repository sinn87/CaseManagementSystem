...
- **审查记录创建逻辑**：
  - 当标签页有字段数据（CaseDetails表中有对应记录）时，创建审查记录
  - 当标签页没有字段数据但有DataGridView数据时，也会创建审查记录
  - 确保所有有内容的标签页都有对应的审查记录，支持后续审查流程
...
## DGV数据表设计
- 每种DGV（如人员表、材料表等）有独立的数据表，字段与DGV列一一对应
- 每张表有自己的编号字段（如：编号、材料编号等），为本表内顺序号或自增，不作为外键
- 所有DGV数据保存采用通用保存方法，按表名和字段动态插入
- 所有DGV表均包含caseID字段用于案件关联

### DataTable行状态管理（新增）
- **数据源绑定**：DataGridView直接绑定DataTable作为DataSource
- **行状态跟踪**：DataTable自动跟踪每行的RowState状态
  - `DataRowState.Added` - 新增行，执行INSERT操作
  - `DataRowState.Modified` - 修改行，执行UPDATE操作
  - `DataRowState.Deleted` - 删除行，执行DELETE操作
  - `DataRowState.Unchanged` - 未变更行，不执行操作
- **自动字段处理**：系统自动填充/更新以下字段
  - 更新时间：记录数据变更时间
  - 更新人员：记录操作人员
  - 审查日期：记录审查时间
  - 审查人员：记录审查人员
  - 状态：根据操作类型设置（新登录/已修改/已删除）
- **事务支持**：所有操作在数据库事务中执行，确保数据一致性
- **性能优化**：批量处理，减少数据库往返次数

## 主要表结构示例

### 人员表（CasePersonItems）
| caseID | 编号 | 姓名 | 性别 | 更新时间 | 审查日期 | 状态 |
|--------|------|------|------|----------|----------|------|

### 材料表（CaseMaterialItems）
| caseID | 编号 | 材料名 | 数量 | 单位 | 更新时间 | 审查日期 | 状态 |
|--------|------|--------|------|------|----------|----------|------|
